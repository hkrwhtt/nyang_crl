...
#웹페이지 열기 (수정)
driver.get("https://ratio.uwayapply.com/Sl5KOlc5SmYlJjomSi1mVGY=")
time.sleep(10)
...
for row in rows:
    ...
    # 기본 케이스들
    if len(values) == 6:
        series, dept, major, num, applicants, ratio = values
        current_series, current_dept, current_major, current_num, current_ratio = series, dept, major, num, ratio
    elif len(values) == 5: # 계열 빠짐
        dept, major, num, applicants, ratio = values
        series = current_series
    elif len(values) == 4:
        # 계열명과 단과대명이 rowspan으로 빠진 경우
        major, num, applicants, ratio = values
        series = current_series
        dept = current_dept
    elif len(values) == 2: # 학과랑 지원인원만 남음
    ...
    # 항상 5개 항목으로 append
    data.append([series, dept, major, num, applicants, ratio])

# pandas DataFrame으로 변환 
# 일단 요소는 5개 다 챙기고 나중에 모집인원과 지원인원을 드랍할 거임
df_old = pd.DataFrame(data, columns=['계열', '단과대명', '학과명', '모집인원', '지원인원', '경쟁률'])
...
filename = "/home/ubuntu/scr_prc/경북대학교_2026_수시_경쟁률_추이.xlsx" # 수정
# filename = r"C:\파이썬 아이들\scr_prc\경북대학교_2025_수시_경쟁률_추이.xlsx"
...
if os.path.exists(filename):
    ...
    df_final = (
        ...
        .drop(columns=["_row_order", "_key", "계열", "모집인원", "지원인원"], errors="ignore")
    )
