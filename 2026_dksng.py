...
#웹페이지 열기 (수정)
driver.get("https://addon.jinhakapply.com/RatioV1/RatioH/Ratio10530541.html")
time.sleep(10)
...
for row in rows:
    ...
    if len(values) == 5:
        ...
        dept = current_dept
    elif len(values) == 6: #좀이따가 밸류들 맞나 보면서 수정ㄱㄱ 그리고 열 이름도 바꿔야함
        # 예체능 전형: 전형명(단과대명), 모집단위, 실기과목, 모집인원, 지원인원, 경쟁률
        dept, major, subject, num, applicants, ratio = values
        major = f"{major} (실기: {subject})"  # 모집단위 + 실기과목 묶기
        current_dept, current_major, current_num = dept, major, num
    else:
...
df_old = pd.DataFrame(data, columns=['전형명', '모집단위', '모집인원', '지원인원', '경쟁률'])
...
df_new["_key"] = df_new["전형명"].astype(str).str.strip() + "_" + \
                 df_new["모집단위"].astype(str).str.strip() + "_" + \
                 df_new["모집인원"]
...
filename = "/home/ubuntu/scr_prc/덕성여자대학교_2026_수시_경쟁률_추이.xlsx"
# 덕성여대는 한 시간 간격!
# filename = r"C:\파이썬 아이들\scr_prc\덕성여자대학교_2025_경쟁률_추이.xlsx"
...
if os.path.exists(filename):
    ...
    df_prev["_key"] = df_prev["전형명"].astype(str).str.strip() + "_" + \
                  df_prev["모집단위"].astype(str).str.strip() + "_" + \
                  df_prev["모집인원"]
    ...
    for col in common_cols:
        if col in ["_key", "전형명", "모집단위", "모집인원"]:  # key 역할인 건 제외
            continue
...
cols = [c for c in df_final.columns if c not in ("전형명", "모집단위")]
final_cols = ["전형명", "모집단위"] + cols
