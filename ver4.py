...
for row in rows:
    ...
    if len(values) == 5:
        ...
        dept = current_dept
        '''elif len(values) == 3:
        # 단과대명과 모집인원(rowspan)이 빠진 경우: [모집, 지원, 경쟁률] 또는 [학과, 지원, 경쟁률] 가능
        # 대부분은 [모집, 지원, 경쟁률] 형식이므로 우선 오른쪽부터 채움
        # (안전하게 오른쪽에서 채워 넣음)
        ratio = values[-1]
        ...
        # 특수 케이스(열 병합 등): 오른쪽에서 차례로 채움, 누락되어있으면 이전값 사용 (얘는 뭔 기능이지;;)
...
'''if os.path.exists(filename):
    ...
    # 기존 데이터도 병합용 키 만들기 
    df_prev["_key"] = df_prev["단과대명"].astype(str) + " " + df_prev["학과명"].astype(str) + " " + df_prev["모집인원"].astype(str)

    # 같은 key가 여러 행이면(과거에 concat 등으로 중복 저장됐을 경우) 한 행으로 축약
    if df_prev.duplicated(subset=["_key"]).any():
        # 각 컬럼에 대해 첫번째 non-null 값을 취함 (시간별 컬럼 병합 목적)
        df_prev = df_prev.groupby("_key", as_index=False).agg(lambda s: s.dropna().iloc[0] if s.dropna().size>0 else "")
    ...
    df_final = (
        ...
        .drop(columns=["_key", "모집인원", "지원인원"], errors="ignore")
    )'''
